...
bot.start(ctx => {
  const menuMessage = `
👋hello, welcome to RanzTELE, press the button below to bring up the menu       

Selamat ${greeting} !
╭━━━「 🌸WELCOME🌸 」━━━⬣
│ 
│ Yo 😉! 
│ Selamat datang di RanzBOT
│
├━━━「 📑 FITUR 」━━━⬣
│ ボタンをクリックしてください
╰━━━━━━━━━━━━━━━━⬣
...
const keyboard = [
    [
        ...
        { text: "☁️ Support Owner", url: "https://t.me/" }
    ],
    [
...
bot.action("thaksto", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello Unknown, this is a creadit from our sc ranz 1.1 thanks to @caywzz because I used the base and sorry if this script is bad because it's only in the development stage.       
...
bot.action("menu", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello Unknown, this is a creadit from our sc Ranz 1.1 thanks to @abee1945 because I used the base and sorry if this script is bad because it's only in the development stage.       
...
bot.action("bugmenu", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
👋 Hello Unknown, I am the 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝘽𝙤𝙩 created by Ranz. I'm here to assist you with anything you might need, making your interaction smoother and more efficient.
Selamat ${greeting}!
╭━━━「 🌸 WELCOME 🌸 」━━━⬣
│ 
│ Yo 😉! 
│ Selamat datang di RanzBOT.1
│
├━━━「 🎯𝘽 𝙐 𝙂 𝙈 𝙀 𝙉 𝙐 」━━━⬣
│ ᝄ ⌜ 𝘽 𝙐 𝙂 𝙈 𝙀 𝙉 𝙐 ⌟
│変-> /xcbeta [62xxx]
